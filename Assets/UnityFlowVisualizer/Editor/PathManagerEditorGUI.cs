...
namespace UnityFlowVisualizer {
    public class PathManagerEditorGUI : EditorWindow {
        ...
        public static void ShowWindow() {
            EditorWindow.GetWindowWithRect(typeof(PathManagerEditorGUI), new Rect(0, 0, 300, 435),false, "Total Path Manager");
        }
        ...
        private void OnGUI() {
            ...
            try {
                ...
                if(Env == null) {
                    ...
                    try {
                        Rect rect0 = EditorGUILayout.GetControlRect(false, 21 * Env.PathGroupList.Count);
                        Rect rect1 = new Rect(rect0.x, rect0.y, 300, 21 * Env.PathGroupList.Count);
                        GUILayout.BeginArea(rect1);

                        for(int i = 0; i < Env.PathGroupList.Count; i++) {

                            Color targetColor = Env.PathGroupList[i].PathColor;
                            string targetName = Env.PathGroupList[i].PathName;
                            GUILayout.BeginHorizontal(EditorStyles.toolbar);
                            GUILayout.Label(ColorToStr(targetColor), BackgroundStyle.GetToolbar(targetColor), GUILayout.Width(69f));
                            Env.PathGroupList[i].PathName = GUILayout.TextField(targetName, EditorStyles.toolbarTextField, GUILayout.Width(165f));
                            Env.PathGroupList[i].gameObject.name = Env.PathGroupList[i].PathName;
                            if (Env.PathGroupList.Count >= 9) { if (GUILayout.Button("Edit", EditorStyles.toolbarButton, GUILayout.Width(49f))) EditButtonClick(i); }
        ...
        public void EditButtonClick(int index) {
            Selection.objects = new UnityEngine.Object[] { Env.PathGroupList[index].gameObject};
            PathGroupEditorGUI.Target = Env.PathGroupList[index];
        ...
        public void NewPathButtonClick() {
            ...
            PathGroup newInfo = newOb.AddComponent<PathGroup>();
